...
function checkPose(prediction, video) {
    ...
    if (prob > 0.8 && !explosionActive) {
        ...
        switch(poseNumber) {
            case '1':
                if (time >= 0.1 && time <= 2.0 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '2':
                if (time >= 2.1 && time <= 4.0 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '3':
                if (time >= 4.1 && time <= 6.6 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '4':
                if (time >= 6.7 && time <= 8.5 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '5':
                if (time >= 8.6 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
        }
